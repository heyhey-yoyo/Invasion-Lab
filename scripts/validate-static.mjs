...
if (packageJson.version !== packageLock.version || packageJson.version !== packageLock.packages?.['']?.version) {
  throw new Error('package.json and package-lock.json root versions differ');
}
if (packageJson.version !== appVersion) throw new Error('package.json and APP_VERSION differ');
if (!modelVersion?.trim()) throw new Error('MODEL_VERSION must identify the scientific model independently of the app release');
...
console.log(`Validated ${ids.length} unique ids, ${manifest.icons.length} manifest icons, ${offlineAssets.length} offline assets, ${scripts.length} JavaScript files, and ${declaredTestCount} declared tests.`);
