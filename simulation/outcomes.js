...
export function classifyOutcome(metrics, config) {
  ...
  if (config.scenarioId === 'leader-follower') {
    ...
    if ((metrics.branchCount ?? 0) >= 2 && passRate > 0.2 && (metrics.leaderFollowerRate ?? 0) < 0.6) return mode('split-migration');
    if (passRate >= 0.3 && (metrics.leaderFollowerRate ?? 0) >= 0.6) return mode('leader-guided');
  ...
  if ((metrics.budCount ?? 0) > 0 && config.presetId === 'budding') return mode('tumor-budding');
...
export function explainOutcome(modeValue, metrics, config, perturbation) {
  ...
  const common = {
    ...
    'single-cell-escape': `这次实验出现单细胞逃逸。较弱细胞连接与较高细胞柔顺性使细胞更容易脱离主体；边界外比例为 ${pct}%。`,
...
export function recommendControl(modeValue, config) {
  const byMode = {
    jammed: '推荐对照：保持细胞行为不变，只放宽几何约束或提高细胞柔顺性。',
    unjamming: '推荐对照：使用同一随机种子撤销扰动，确认速度跃迁来自局部松动而非随机波动。',
    'collective-advance': '推荐对照：降低细胞连接，观察连续推进是否转变为出芽或分散逃逸。',
    'tumor-budding': '推荐对照：提高邻居对齐，观察小型细胞芽是否重新并入主体。',
    'single-cell-escape': '推荐对照：提高细胞连接，观察分散逃逸是否转变为集体迁移。',
    fingering: '推荐对照：分别提高细胞柔顺性与开口宽度，比较哪个因素更能促成完整通过。',
    'stable-cluster': '推荐对照：只松动边缘细胞连接，观察主体边界是否开始形成小型细胞簇。',
    ...
    'collective-flow': '推荐对照：提高密度或降低细胞柔顺性，观察系统是否重新进入拥堵。'
  };
